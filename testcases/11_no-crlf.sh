...
# Description
#   Execute the CalliopEO.py script with a hex file sending data without
#   newlines. CalliopEO.py should handle this situation by truncating the
#   lines.
# Preparation
#   Provide zip archives 15sec-burst-no-crlf.zip.
# Expected result
#   CalliopEO.py returns code 0
#   CalliopEO.py renames the the .zip to .zip.done
#   CalliopEO.py creates a folder run_*
#   CalliopEO.py creates .data files in folder run_*
#   The created .data file has correct MD5 checksum
# Necessary clean-up
...
###############################################################################
zipfile="01.zip"
hexfile1="testcases/testfiles/15sec-burst-no-crlf.hex"
datafile1="testcases/testfiles/15sec-burst-no-crlf.hex.data"
...
###############################################################################
echo "Test: Handle data without newline"
echo "---------------------------------"
echo ""
...
# Created .data files in the folder run_*?
data_files=($(find ./run_* -name "*.data"))
echo -n "Check 4/6: Created .data files ... "
